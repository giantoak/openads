...
public class MemexAd {
	...
	public static HashMap<Integer,MemexAd> fetchAdsOculus(Connection htconn, Connection oculusconn, String whereClause, boolean includeFirstId) {
		HashMap<Integer,MemexAd> result = new HashMap<Integer,MemexAd>();
		String sqlStr = "SELECT ads_id,value from ads_emails WHERE " + whereClause;
		Statement stmt = null;
		try {
		...
		sqlStr = "SELECT ads_id,value from ads_websites WHERE " + whereClause;
		stmt = null;
		try {
			stmt = oculusconn.createStatement();
			ResultSet rs = stmt.executeQuery(sqlStr);
		...
		if (includeFirstId) {
			sqlStr = "SELECT id,first_id from ads WHERE " + whereClause.replace("ads_id","id");
			stmt = null;
			try {
				stmt = htconn.createStatement();
				ResultSet rs = stmt.executeQuery(sqlStr);
		...
		sqlStr = "SELECT ads_id,value from " + AdExtraction.ADS_PHONE_TABLE + " WHERE " + whereClause;
		stmt = null;
		try {
			stmt = oculusconn.createStatement();
			ResultSet rs = stmt.executeQuery(sqlStr);
